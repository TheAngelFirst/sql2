...
begin
	select PRODUCT_NAME into cheapest_gpa from PIES_SHAK where ROWNUM <= 1 and MAKER_TYPE = 'дедушка' order by PRICE asc;
	select PRODUCT_NAME into cheapest_gma from PIES_SHAK where ROWNUM <= 1 and MAKER_TYPE = 'бабушка' order by PRICE asc;
